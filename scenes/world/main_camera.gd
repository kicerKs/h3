...
func focus(pos: Vector2):
	var off = get_node("/root/Main/GUI").get_gui_size()
	position.x = clamp(pos.x + off.x/2 + 31, limit_left+get_viewport().size.x/2, limit_right-get_viewport().size.x/2)
	position.y = clamp(pos.y + off.y - 31, limit_top+get_viewport().size.y/2, limit_bottom-get_viewport().size.y/2)
